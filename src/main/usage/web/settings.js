...
function translate() { document.documentElement.lang = language; document.querySelectorAll('[data-i18n]').forEach(e => e.textContent = language === 'en-US' ? english[e.dataset.i18n] || chinese.get(e.dataset.i18n) : chinese.get(e.dataset.i18n)); $('language').textContent = tr('English', '中文'); $('heading').textContent = selected?.id ? selected.displayName : tr('添加 AI 账号', 'Add an AI account'); identityView(); nav(); if (selected) periodRows(readPeriods()); }
...
async function loadSnapshots() { if (polling || flow)
    ...
    if (snapshot) renderResult(snapshot);
...
$('form').onsubmit = e => { e.preventDefault(); void action(async () => { const saved = await api('connections', 'POST', { connection: read(), credential: credential() }); await reload(saved); message(tr('已保存。查询结果将在 Pet 面板更新。', 'Saved. Usage will update on your Pet.')); }); };
$('test').onclick = async () => {
    if (testBusy) return;
    ...
    try {
        ...
        renderResult(result, $('test-results'));
