...
- 右键宠物可新增 TODO、打开 Markdown 源文件、导入宠物 zip、刷新宠物、切换宠物样式和退出。
- 右键 TODO 可标记完成、删除、标红或取消标红。
- 长按 TODO 可在同一天内调整排序。
- TODO 完成后会划线并移动到当天列表末尾。
- 宠物状态会跟随 TODO 状态切换：有未完成 TODO 时为 `review`，清空后为 `idle`，新增后短暂 `waving`，拖拽时根据方向显示 `running-left` 或 `running-right`。
